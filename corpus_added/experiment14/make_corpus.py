"""Experiment 14: a large generated corpus restricted to the Experiment 6 vocabulary.
...
for t, (c1, c2) in itertools.product(things, itertools.permutations(colors, 2)):
    if t not in {"box"} and rng.random() < 0.3:
...
for folder in [ROOT / "corpus", HERE]:
    ...
    (folder / "generated_5k.txt").write_text("\n".join(kept) + "\n", encoding="utf-8")
